...
func NewEndpointSliceCache(hostname string, recorder record.EventRecorder, makeEndpointInfo makeEndpointFunc) *EndpointSliceCache {
...
func (cache *EndpointSliceCache) addEndpointsByIP(serviceNN types.NamespacedName, portNum int, endpointsByIP map[string]Endpoint, endpoints []*endpointInfo) map[string]Endpoint {
	...
	for _, endpoint := range endpoints {
		...
		endpointInfo := newBaseEndpointInfo(v1.IPv4Protocol, v1.ProtocolTCP, endpoint.Addresses[0], portNum, isLocal, endpoint.Topology)
