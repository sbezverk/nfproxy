...
func NewServiceChangeTracker(makeServiceInfo makeServicePortFunc, recorder record.EventRecorder) *ServiceChangeTracker {
...
// UpdateService updates given service's change map based on the <previous, current> service pair.  It returns true if items changed,
...
func (sct *ServiceChangeTracker) UpdateService(previous, current *v1.Service) bool {
	svc := current
	if svc == nil {
		svc = previous
	}
