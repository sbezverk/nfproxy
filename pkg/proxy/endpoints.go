...
func NewEndpointChangeTracker(hostname string, makeEndpointInfo makeEndpointFunc, recorder record.EventRecorder, endpointSlicesEnabled bool) *EndpointChangeTracker {
	...
	if endpointSlicesEnabled {
		ect.endpointSliceCache = NewEndpointSliceCache(hostname, recorder, makeEndpointInfo)
	}
	return ect
}

// UpdateEndpoint updates given service's endpoints change map based on the <previous, current> endpoints pair.  It returns true
...
func (ect *EndpointChangeTracker) UpdateEndpoint(previous, current *v1.Endpoints) bool {
	endpoints := current
	if endpoints == nil {
		endpoints = previous
	}
	...
	for i := range endpoints.Subsets {
		...
		for i := range ss.Ports {
			...
			for i := range ss.Addresses {
				...
				baseEndpointInfo := newBaseEndpointInfo(v1.IPv4Protocol, port.Protocol, addr.IP, int(port.Port), isLocal, nil)
